...
plugins {
    ...
    `maven-publish`   // this addon publishes a type other addons extend — see Liberation
}
...
// ---------------------------------------------------------------------------
// TheftOfFire opens an extension point (Liberation), so other addons need its type at
// COMPILE time. They get it the same way they get the host API:
//
//   build.gradle.kts:  compileOnly("net.crewco:theft-of-fire:0.1.0")
//   addon.yml:         depends: [ TheftOfFire ]
//
// At RUNTIME the class comes out of the loaded TheftOfFire jar — the host's
// AddonClassLoader delegates to whatever an addon names in `depends:`, so there is
// exactly one Liberation class and `instanceof` works across addons. Never shade it.
//
